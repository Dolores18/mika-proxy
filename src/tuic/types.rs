use crate::flow::{DestinationAddr, Resolver, Buffer};
use anyhow::anyhow;
...
pub struct UdpSession {
    pub incoming: tokio::sync::mpsc::Sender<crate::flow::Buffer>,
    pub local_addr: crate::flow::DestinationAddr,
...
impl UdpSession {
    ...
    pub fn new(incoming: tokio::sync::mpsc::Sender<crate::flow::Buffer>, local_addr: crate::flow::DestinationAddr) -> Self {
    ...
    pub fn is_expired(&self, timeout: std::time::Duration) -> bool {
        std::time::Instant::now().duration_since(self.last_active) > timeout
    }
}
...
impl ServerAddr {
    ...
    pub async fn resolve(
        ...
        if let Some(ip) = self.ip {
            ...
            let ips = resolver
                .resolve_ipv4(self.domain.clone())
                .await?;
            if ips.is_empty() {
                return Err(anyhow!("Resolve failed: unknown hostname"));
            }
            Ok(SocketAddr::from((ips[0], self.port)))
        }
    }
}
...
impl SocketAdderTrans for crate::flow::DestinationAddr {
    fn into_tuic(self) -> tuic::Address {
        match self.host {
            crate::flow::HostName::Ip(addr) => tuic::Address::SocketAddress(SocketAddr::new(addr, self.port)),
            crate::flow::HostName::DomainName(domain) => {
                tuic::Address::DomainAddress(domain, self.port)
            }
        }
    }
}