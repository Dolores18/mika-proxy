    ...
    GenericArray<u8, Inner::NonceSize>: Send + Sync + Unpin + 'static,
{
    ...
    fn encrypt(
        ...
        info!(
            "开始用aead算法进行加密， pre_overhead的长度是{}",
            pre_overhead.len()
        );
        let tag: GenericArray<
            u8,
            cipher::typenum::UInt<
                cipher::typenum::UInt<
                    cipher::typenum::UInt<
                        cipher::typenum::UInt<
                            cipher::typenum::UInt<cipher::typenum::UTerm, rc4::consts::B1>,
                            rc4::consts::B0,
                        >,
                        rc4::consts::B0,
                    >,
    ...
    fn decrypt_size(
        ...
        info!(
            "服务端开始验证原始数据包的大小， 使用的是pre_overload{:0X?}",
            pre_overhead
        );
        let (size_buf, size_tag) = pre_overhead.split_at_mut(2);
        info!("成功分割pre_overhead");
        if self
            .inner
