...
pub struct ShadowsocksSeverStream<C: ShadowCrypto>
where
    [(); C::KEY_LEN]:,
{
...
impl<C: ShadowCrypto + Unpin> Stream for ShadowsocksSeverStream<C>
where
    [(); C::KEY_LEN]:,
    [(); C::IV_LEN]:,
    ...
    [(); C::POST_CHUNK_OVERHEAD]:,
{
    ...
    fn poll_rx_buffer(
        ...
        println!("ss服务端开始加密响应数据");
        let Self {
            lower,
            rx_buf: rx_buf_opt,
            rx_chunk_size,
            tx_crypto: crypto, // 使用 tx_crypto 进行加密
            reader,
            ..
        } = &mut *self;
        ...
        let res = if C::POST_CHUNK_OVERHEAD == 0 {
            ...
            let res = ready!(reader.poll_peek_at_least(cx, lower.as_mut(), 1, |buf| {
                ...
                crypto.encrypt(
                    &mut [0; C::PRE_CHUNK_OVERHEAD],
                    buf,
                    &mut [0; C::POST_CHUNK_OVERHEAD],
                ); // 加密数据
            ...
            let res = ready!(
                reader.poll_read_exact(cx, lower.as_mut(), chunk_size, |buf| {
                    let mut encrypted = vec![0u8; buf.len() + C::POST_CHUNK_OVERHEAD];
                    let (pre_overhead, chunk) = encrypted.split_at_mut(C::PRE_CHUNK_OVERHEAD);
                    let (chunk, post_overhead) = chunk.split_at_mut(buf.len());
                    crypto.encrypt(
                        pre_overhead.try_into().unwrap(),
                        buf,
                        post_overhead.try_into().unwrap(),
                    );
                    rx_buf.extend_from_slice(&encrypted);
                    true
                })
            );
            res.map(|_| ())
    ...
    fn poll_tx_buffer(
        ...
        let buf = ready!(lower.as_mut().poll_tx_buffer(cx, size))?;
        *tx_offset = buf.len();
        Poll::Ready(Ok(buf))
    }

    fn commit_tx_buffer(&mut self, mut buffer: Buffer) -> FlowResult<()> {
        println!("ss服务端开始解密发送数据");
        let Self {
            lower,
            rx_crypto: crypto,
            tx_offset,
        ...
        let crypto = match crypto {
            RxCryptoState::ReadingIv { .. } => return Err(FlowError::UnexpectedData),
            RxCryptoState::Ready(c) => c,
        };

        let part = &mut buffer[*tx_offset..];
        if C::POST_CHUNK_OVERHEAD == 0 {
            // Stream cipher
            let _ = crypto.decrypt(part, &[0; C::POST_CHUNK_OVERHEAD]);
        } else {
            // AEAD cipher
            ...
            buffer.truncate(buffer.len() - C::POST_CHUNK_OVERHEAD);
        }
