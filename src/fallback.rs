...
pub struct FallbackStream<F: FnOnce(Box<dyn Stream>) + Unpin> {
    pub tx_closed: bool,
    pub lower: ManuallyDrop<Box<dyn Stream>>,
    pub on_fallback: ManuallyDrop<F>,
}
...
impl<F: FnOnce(Box<dyn Stream>) + Unpin> FallbackStream<F> {
    pub fn new(lower: Box<dyn Stream>, on_fallback: F) -> Self {
        Self {
            tx_closed: false,
            lower: ManuallyDrop::new(lower),
            on_fallback: ManuallyDrop::new(on_fallback),
        }
    }
}
