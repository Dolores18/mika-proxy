...
impl AsyncRead for CompatStream {
    fn poll_read(
        ...
        //log::info!("CompatStream: Attempting to read data");
        ...
        let res = ready!(reader.poll_peek_at_least(cx, &mut **inner, 1, |data| {
            ...
            //println!("CompatStream: Read {} bytes", to_read);
            to_read
        }))
        .or_else(|e| {
            if let FlowError::Eof = &e {
                //println!("CompatStream: Reached EOF");
                Ok(0)
            } else {
                //println!("CompatStream: Error reading - {:?}", e);
                Err(e)
            }
        })
...
impl AsyncWrite for CompatStream {
    fn poll_write(
        ...
        //println!("CompatStream: Attempting to write {} bytes", buf.len());
        ...
        //println!("CompatStream: Wrote {} bytes", len.get());
