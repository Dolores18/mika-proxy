use proxy::{start_udp_server, ServerConfig, StatHandle};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("启动 UDP 代理服务器...");

    // 调用库中的start_udp_server函数，使用新的函数签名
    start_udp_server(server_addrs, config, app_config).await?;
