use proxy::{start_dispatcher_server, ServerConfig, StatHandle};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 使用新的函数签名调用start_dispatcher_server
    start_dispatcher_server(server_addrs, config, app_config).await?;
